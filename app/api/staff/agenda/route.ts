...
const STAFF_AGENDA_TURNO_SELECT = [
  "id",
  "servicio_id",
  "servicio_final_id",
  "empleada_id",
...
export async function GET() {
  try {
    ...
    const { data: turnosData, error: turnosError } = await db
      .from("turnos")
      .select(STAFF_AGENDA_TURNO_SELECT)
    ...
    const sugerencias =
      staff?.activo === false
        ...
        : buildStaffTurnosOfrecidos({
            ...
            localHorarios: Array.isArray((configLocal as any)?.horario_local)
              ? (configLocal as any).horario_local
              : [],
    ...
    console.error("GET /api/staff/agenda failed", error)
    return NextResponse.json({ error: "No se pudo cargar la agenda de staff." }, { status: 500 })
  }
}
